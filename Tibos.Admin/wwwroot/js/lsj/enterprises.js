//请求地址(列表)
var RequestListUrl = "/Enterprises/GetEnterprise_infoList";
//请求地址(单条记录)
var RequestUrl = "/Enterprises/GetEnterprise_info";
//请求地址(删除记录)
var RequestDelUrl = "/Enterprises/DelEnterprise_info";
//请求地址(修改)
var RequestEditUrl = "Enterprises/EditEnterprise_info";
//请求控制器(添加)
var RequestAddUrl = "Enterprises/AddEnterprise_info"


//请求地址(添加2)
var RequestAddUrl2 = "Enterprises/AddEnterprise_info2";
...
$(function () {
    ...
    //3 追加按钮
    $(".columns.columns-right.btn-group.pull-right").append("<button class=\"btn btn-default\" type=\"button\" name=\"refresh\" title=\"详细\" onclick=\"btn_detailed()\"><i id=\"detailed\" class=\"fa fa-search-plus\"></i></button>");
});
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_departments').bootstrapTable({
            ...
            columns: [{
                ...
                valign: 'middle',//水平居中
                //sortable: true,//是否可排序                
                formatter: nameFormatter //初始化,自定义内容(厉害了,word哥,有了这个,想怎么搞就怎么搞了)
    ...
    function nameFormatter(value, row, index) {
        var res = row.name;
        var c = $("#detailed").attr("class");
        if (c != "fa fa-search-plus") {
            if (row.name != null && row.name.length > 4) {
                res = row.name.substring(0, 4) + "...";
            }
        }
        return res;
    }
    ...
    function addressFormatter(value, row, index) {
        ...
        if (c != "fa fa-search-plus")
        {
            if (row.address != null && row.address.length > 4) {
                res = row.address.substring(0, 4) + "...";
            }
        }
        return res;
    }
    ...
    function remarkFormatter(value, row, index) {
        ...
        var c = $("#detailed").attr("class");
        if (c != "fa fa-search-plus") {
            if (row.remark != null && row.remark.length > 4) {
                res = row.remark.substring(0, 4) + "...";
            }
        }
        return res;
    }
    function job_postFormatter(value, row, index) {
        var res = row.job_post;
        var c = $("#detailed").attr("class");
        if (c != "fa fa-search-plus") {
            if (row.job_post != null && row.job_post.length > 4) {
                res = row.job_post.substring(0, 4) + "...";
            }
        }
        return res;
    }
    ...
    function commonFormatter(value, row, index) {
        //var res = "<button id='btn_edit' type='button' class='btn btn-warning'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span>编辑</button>";
        var res = "<button id=\"\" type=\"button\" class=\"btn btn-danger\" onclick='btn_common(" + row.id + ")' style=\"height:24px;padding: 0px 8px;margin: 0px 8px; */\"><span class=\"fa fa-glass\" aria-hidden=\"true\"></span>释放</button>";
        return res;
    }
    ...
    oInit.Init = function () {
        ...
        $("#btn_edit").click(function () {
            ...
            $.post(RequestUrl, { id: arrselections[0].id }, function (data) {
                ...
                if (data.job_terrace != null) {
                    $("#chk2").find(".chkbox").each(function () {
                        if (data.job_terrace.indexOf($(this).children(".radiobox-content").html()) >= 0) {
                            $(this).prev().prop("checked", "checked");
                            $(this).removeClass("unchecked");
                            $(this).addClass("checked");
                            $(this).children(".check-image").css("background", "url(../img/input-checked.png)");
...
function btn_common(id) {
    var d = dialog({
        ...
        ok: function () {
            this.title('提交中…');
            $.post("Enterprises/GetEnterprise_Common", { id: id }, function (data) {
                var e = dialog({
                    padding: 30,
                    content: data.msg
                });
...
function btn_detailed()
{
    var c = $("#detailed").attr("class");
    if (c == "fa fa-search-plus") {
        $("#detailed").attr("class", "fa fa-search-minus");
    } else
    {
        $("#detailed").attr("class", "fa fa-search-plus");
    }
    $("#tb_departments").bootstrapTable('refresh');
}
