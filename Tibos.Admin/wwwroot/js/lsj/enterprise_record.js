//请求地址(列表)
var RequestListUrl = "/Enterprise_record/GetEnterprise_recordList";
//请求地址(单条记录)
var RequestUrl = "/Enterprise_record/GetEnterprise_record";
//请求地址(删除记录)
var RequestDelUrl = "/Enterprise_record/DelEnterprise_record";
//请求地址(修改)
var RequestEditUrl = "Enterprise_record/EditEnterprise_record";
//请求控制器(添加)
var RequestAddUrl = "Enterprise_record/AddEnterprise_record"
    ...
    oTableInit.Init = function () {
        $('#tb_departments').bootstrapTable({
            ...
            columns: [{
                ...
                valign: 'middle',//水平居中
                //sortable: true,//是否可排序                
                formatter: enterpriseFormatter //初始化,自定义内容(厉害了,word哥,有了这个,想怎么搞就怎么搞了)
        ...
        $("#btn_edit").click(function () {
            ...
            $.post(RequestUrl, { id: arrselections[0].id }, function (data) {
                ...
                if (data.enterprise != null) {
                    $('#txt_enterprise').selectpicker('val', data.enterprise.id);
                    $("#txt_enterprise").val(data.enterprise.id);
                }
                dean(data.enterprise.id);
                $("#txt_dean").find("option[value='" + data.dean + "']").attr("selected", true);
...
$(function () {
    //绑定事件
    $("#txt_enterprise").change(function () {
        var provice = $('#txt_enterprise option:selected').val();
        //判断省份这个下拉框选中的值是否为空
        if (provice == 0) {
            $("#txt_dean").html("");
...
function dean(provice) {
    $("#txt_dean").html("");
    var str = "<option value=0>==请选择===</option>";
    $.ajax({
        type: "POST",
        url: "/Enterprise_record/GetAddress",
        data: { "id": provice },
        dataType: "JSON",
        async: false,
